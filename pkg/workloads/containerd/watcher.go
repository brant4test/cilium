...
func syncWithRuntime() {
	...
	// FIXME: Must be synchronize with event handler
...
// backgroundContainerSync is a go routine which periodically synchronizes
// containers managed by the local container runtime and checks if any of them
// need to be managed by Cilium. This is a fall back mechanism in case an event
// notification has been lost.
func backgroundContainerSync() {
	for {
...
func listenForDockerEvents(reader io.ReadCloser) {
	...
	for scanner.Scan() {
		...
		if e.ID != "" {
			log.Debugf("Processing docker event %v for container %v", e.Status, e.ID)
...
func handleCreateContainer(id string, retry bool) {
	...
	for try := 1; try <= maxTries; try++ {
		...
		if dockerContainer.NetworkSettings != nil {
			ep.SetDockerEndpointID(dockerContainer.NetworkSettings.EndpointID)
		}
		...
		err = workloads.Owner().EndpointLabelsChanged(ep, identityLabels, informationLabels)
		if err != nil {
